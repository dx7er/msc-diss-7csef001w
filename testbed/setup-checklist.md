...
**Backup directory (host):** `D:\UOW\SEM3\Backups\`
...
**Records.** Copy `Windows-Version.txt`, `Whoami.txt`, `Whoami-User.txt`, `Local-Users.txt` from `C:\DISS_Config\` to `testbed/config-snapshots/step-02/` in the repo.
...
**Records.** `Locale.txt`, `Language-List.txt`. Update `windows-configuration.md` with computer name, timezone, locale, language.
...
**Records.** `Installed-Hotfixes.txt`, `Defender-Version.txt`, `VMware-Tools-Version.txt`. Copy to `testbed/config-snapshots/step-04/`.
...
**Records.** Save BitLocker status output and note power settings in `windows-configuration.md`.
...
**Records.** `audit-before.csv`, `audit-after.csv`, `audit-policy-backup.csv`, `audit-subcategories.txt`. Copy to `testbed/config-snapshots/step-06/`. Update `windows-configuration.md` with the enabled subcategory list.
...
**Records.** All XML configs and `USB-Event-Channels.txt`. Copy to `testbed/config-snapshots/step-07/`.
...
**Records.** Screenshot or `Out-File` the three commands' output to `Prefetch-Config.txt`. Copy to `testbed/config-snapshots/step-08/`.
...
**Records.** Update `windows-configuration.md` with the test-data root path and the token-naming scheme.
...
**Records.** Screenshot every VMware Settings tab. Copy to `testbed/config-snapshots/step-10/`.
...
**Records.** Note the tools used to mount and hash, and store the acquisition manifest in `testbed/config-snapshots/step-13/`.
...
**Records.** Pilot ground-truth CSVs, host acquisition logs, parsed output. Store under `scenarios/P00-pilot/` (do not commit raw artefacts; commit hashes and parsed CSVs).
...
Steps 2 to 5 in order. Do not create the formal snapshot until Steps 6 to 10 are also complete and pilots (Step 14) have passed.
