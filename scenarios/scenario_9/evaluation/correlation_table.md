...
| A02 | 09:04:39 | Navigate to UoW website | CHROME.EXE x5 (helper spawns for the new navigation); TASKHOSTW.EXE (background) | Security 4688 x80 with parent services.exe, 4689 x74, CAPI2 EID 4097 naming CN=GlobalSign Root CA R3 (the westminster.ac.uk TLS chain) | no match | PARTIAL | Downgraded from CONFIRMED in the 6 Sep 2026 record-level verification (see `scripts/evaluation/evaluation_matrix_corrections.md`). The CAPI2 4097 record carries a certificate-authority name and thumbprint only, not a hostname, and none of the 654 windowed records names westminster.ac.uk. GlobalSign issues certificates for many sites, so the navigation is consistent with the ground truth but the destination is not identified. |
