...
> **Record-level verification, 6 September 2026.** Every cell whose note asserted a specific record was re-checked against the parsed output. Eight verdict cells changed and five notes were rewritten. This file and `evaluation_matrix_corrected.csv` carry the corrected figures used in the dissertation. What changed, and why, is in [`evaluation_matrix_corrections.md`](evaluation_matrix_corrections.md). The original coding is kept in `evaluation_matrix.csv` and in git history so that the pre-verification version stays inspectable.
...
Machine-readable mirror at `evaluation_matrix_corrected.csv` (the pre-verification coding is kept in `evaluation_matrix.csv`). Per-scenario detail at `scenarios/scenario_N/evaluation/correlation_table.md` (14 tables in total: eight single-run, plus three per-run tables each for the multi-run scenarios S04 and S07 (executed 3 runs each)).
...
|----------|---------------------------------------------------------|------|------|-------|------------|-----------------|
| S01      | Chrome installer executed                               | PASS | PASS | N/A   | PASS       | N/A             |
| S01      | WinRAR installer executed                               | PASS | PASS | N/A   | PASS       | N/A             |
| S01      | VLC installer executed                                  | PASS | PASS | N/A   | PASS       | N/A             |
| S01      | Adobe Reader installer executed                         | PASS | PASS | N/A   | PASS       | N/A             |
| S01      | Zoom installer executed                                 | PASS | PASS | N/A   | PASS       | N/A             |
| S01      | Notepad++ installer executed                            | PASS | PASS | N/A   | PASS       | N/A             |
| S01      | Chrome install completed successfully                   | PASS | PASS | N/A   | PASS       | N/A             |
| S01      | WinRAR install completed successfully                   | PASS | PASS | N/A   | PASS       | N/A             |
| S01      | VLC install completed successfully                      | PASS | PASS | N/A   | PASS       | N/A             |
| S01      | Adobe Reader install completed successfully             | PASS | PASS | N/A   | PASS       | N/A             |
| S01      | Zoom install completed successfully (+ service 7045)    | PASS | PASS | N/A   | PASS       | N/A             |
| S01      | Notepad++ install completed successfully                | PASS | PART | N/A   | PART       | N/A             |
...
| S02      | notepad++.exe launched (3 times)                        | PASS | PASS | N/A   | PASS       | N/A             |
| S02      | Prefetch run count = 3 per app                          | PART | N/A  | N/A   | PART       | N/A             |
...
| S04      | USB device attached (VID/PID/serial)                    | N/A  | PASS | N/A   | PASS       | 3/3             |
...
| S05      | File deleted to Recycle Bin                             | FAIL | FAIL | N/A   | FAIL       | N/A             |
| S05      | Recycle Bin emptied                                     | PART | PART | N/A   | PART       | N/A             |
...
| S07      | File scenario7_test_file.txt saved to Documents root    | PASS | N/A  | PASS  | PASS       | 3/3             |
...
| S09      | URL 1 visited (westminster.ac.uk)                       | N/A  | PART | N/A   | PART       | N/A             |
| S09      | URL 2 visited (bbc.co.uk)                               | N/A  | PART | N/A   | PART       | N/A             |
| S09      | URL 3 visited (Wikipedia digital forensics)             | N/A  | PART | N/A   | PART       | N/A             |
| S09      | File downloaded                                         | PASS | PASS | N/A   | PASS       | N/A             |
| S09      | Downloads folder opened in Explorer                     | PART | N/A  | PASS  | PASS       | N/A             |
...
- S07 row "File saved to Documents root" is scored PASS on Prefetch (`notepad++.exe` RunTime) and ShellBags (`UsrClass` Desktop\Documents written by the Save As dialog), with 3/3 reproducibility. EVTX is `N/A`: every `4663` record inside the Save As windows names `\Device\CdRom0\` or `\Device\CdRom0\setup.exe`, not the saved file, and file-system auditing was not enabled for profile paths. The save is therefore supported by an interpreted sequence, not by a direct record of the write.
...
- S10 A03 (logon after boot) is scored FAIL under strict time-window semantics: the boot-side EVTX events (EID 20 / 6005 / 6009 / 4608 / 7001) fired at the guest's clock time, which drifted from the host wall-clock across the shutdown-and-boot cycle. The guest-versus-host offset is 251.4 s at A02 and 172.0 s at A03, so no constant correction exists and a per-scenario offset would not justify an upgrade.
...
2. **Multi-class agreement**: rows where at least two of the three single-artefact classes independently PASS (the correlated verdict is supported by two or three independent evidential streams that agree on the same ground-truth item). Count: **24 of 35 = 68.6%**. All 24 rows carry two independent PASSes (typically Prefetch and EVTX agreeing on an execution event; or ShellBags and EVTX agreeing on a folder-navigation event). No row carries all three classes PASS: the S07 file-save row, previously scored PASS on all three, lost its EVTX PASS in the 6 September verification because the cited 4663 records name the CD-ROM device, not the saved file. Multi-class agreement is the property a marker or examiner should read as "correlation works": two or three independent artefact streams converge on the same interpretation of the same action.

3. **Single-class carry**: rows where exactly one artefact class was PASS. Count: **11 of 35 = 31.4%**. In ten of these the other two classes are `N/A` because they cannot structurally record the item; in the eleventh (S09 Downloads folder opened) ShellBags PASS is supported by a Prefetch PART. EVTX-only rows are S04 USB attach and eject, S06 logon and logoff (`4624`, `4634`) and S08 `whoami` invocation (`4688`); ShellBags-only rows are the S03 level-5 and negative-control rows, the S04 USB subfolder, the two S07 Save As dialog rows and S09 Downloads. Single-class carry rows are not correlation failures; they are cases where the scenario deliberately exercises one artefact class, and the class delivered.
...
S02 (application-launch baseline) is the ceiling case: application launch is what Prefetch and Security 4688 were designed to capture, so all six launch rows are directly attributed. The seventh row (Prefetch run count = 3 per app) is PART because Notepad's current entry shows 4 and Chrome carries several entries with different counts. S04 (USB lifecycle) is the strongest realistic scenario: 4 of 5 rows PASS with the only PART attributable to a windowing-timing variance rather than a coverage gap. S05 (delete to Recycle Bin) is the floor case: two of two rows below PASS because delete-to-Recycle-Bin and Empty-Bin actions are genuinely outside the three-artefact scope and require a fourth class (`$I`/`$R` filesystem parsing). S03 (nested navigation) shows a scoring artefact rather than a real gap: all six ShellBag rows are PASS at the class level but Correlated is scored PART for four of them because only one artefact class contributes; the scenario design deliberately isolates ShellBags as the single attribution surface, so this is exactly the expected pattern.
...
1. Install actions are the study's strongest cross-artefact case (11 of the 12 S01 installer rows PASS on both Prefetch and EVTX) because installer binaries execute (Prefetch), spawn under user Explorer (EVTX 4688 with parent explorer.exe), and register services (EVTX 7045) or run MSI sessions (EVTX MsiInstaller). For the NSIS installer (Notepad++), execution is directly evidenced by `4688` records 34111 and 34119, but install completion drops to PART because there is no MsiInstaller trail; the difference is visible only when the matrix is scanned column-by-column across all install rows.

2. Pure browser downloads, delete-to-Recycle-Bin, workstation lock/unlock, and shell-built-in commands are four distinct below-PASS cases (S01 A02-A07, S05 A02, S06 A03/A05, S08 A03/A06) that share a common root cause: each is a shell-level or protocol-level action that does not spawn a new binary, does not touch the shell namespace, and does not fire a distinctive Security-auditing event on the study's baseline audit policy. This is not four separate coverage gaps but one recurring gap in the three-artefact scope. Lock and unlock are the exception to the root cause: Security 4800 and 4801 are present, 3.5 s after and 34 s before their windows, so those two rows are window-boundary failures rather than artefact absence.
